...
*/
function random_choices(min, max, n) {
	min = Math.ceil(min);
	max = Math.floor(max);
	let N = max - min + 1;
	if(N < n) {
...
*/
function get_randomly_multi(arr, n) {
...
/*
	...
	중복이 허용된다.
*/
function get_randomly_multi_dup(arr, n) {
	let out = [];
	for(let i = 0; i < n; ++i)
		out[i] = get_randomly(arr);
	return out;
}