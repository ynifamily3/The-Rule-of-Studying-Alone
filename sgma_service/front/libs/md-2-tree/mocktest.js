...
Mocktest.select_test_materials = function(roots, n) {
  ...
  for (let i = 0; i < m; ++i) ratio[i] = quotient;
  n -= quotient * m;
  ...
  for (let i = 0; i < ratio.length; ++i)
    for (let k = 0; k < ratio[i]; ++k) out.push(subinfos[i]);
...
Mocktest.create_mocktest = function(roots, n) {
  ...
  for (let k = 0; k < n; ++k) {
    ...
    if (quest_types[type_ptr]-- <= 0) ++type_ptr;
    ...
    try {
      if (type_ptr == 0)
        ...
        new_quest = Quest.generate_selection_quest(
          domains[k],
          4,
          1,
          Math.random() > 0.5
      ...
      else throw new Error("Illegal Quest Type Included: " + type_ptr);
    } catch (err) {
